...
def process_data():
    try:
        ...
        for c in range(11, 19):
            ...
            if not h_ch:
                print(f"Chapter {c} not found in hindi_data")
                continue
            ...
            for n in nums:
                h_v = hindi_data.get('verses', {}).get(c_str, {}).get(n)
                e_v = english_data.get('verses', {}).get(c_str, {}).get(n) if english_data else None
                ...
                if h_v:
                    ...
                    try:
                        num_val = int(h_v.get('verse_number', '0').split('-')[0])
                        if num_val == 0:
                    ...
                    print(f"Verse {n} not found in chapter {c}")
