...
import { chapter11Shlokas } from "../../../data/chapter11";
...
const Adhyay11 = () => {
  ...
  useEffect(() => {
    if (currentUser) {
      const fetchBookmarks = async () => {
        ...
        if (docSnap.exists()) {
          setBookmarks(docSnap.data().chapter11 || []);
        } else {
          setBookmarks([]);
        }
        setIsLoading(false);
  ...
  const toggleBookmark = async (shlokaNum) => {
    ...
    try {
      const docRef = doc(db, "userBookmarks", currentUser.uid);
      await setDoc(docRef, { chapter11: newBookmarks }, { merge: true });
    } catch (error) {
  ...
  return (
    <div className="bg-gradient-to-br from-amber-50/30 via-[#F8F5F2] to-orange-50/20 dark:from-slate-950 dark:via-slate-900 dark:to-slate-950 text-slate-800 dark:text-slate-100 min-h-screen font-sans transition-colors duration-300">
      <SEO 
        title={`Bhagavad Gītā Chapter 11: विश्वरूपदर्शनयोगः - Darshanam`} 
        description={`Explore Bhagavad Gītā Chapter 11 - Vishwarupa Darshana Yoga - The Universal Form. Read translations, listen to audio, and discover the meaning.`} 
      />
      <SEO 
        title={`Bhagavad Gītā Chapter 11: विश्वरूपदर्शनयोगः - Darshanam`} 
        description={`Explore Bhagavad Gītā Chapter 11 - Vishwarupa Darshana Yoga - The Universal Form. Read translations, listen to audio, and discover the meaning.`} 
      ...
      >
        <div className="relative container mx-auto px-4">
          ...
          >
            Vishwarupa Darshana Yoga - The Universal Form
          </motion.h2>
          <motion.div
