...
// --- Data for Chapter 2 (Sankhya Yoga) ---
...
const Adhyay2 = () => {
  ...
  useEffect(() => {
    if (currentUser) {
      const fetchBookmarks = async () => {
        ...
        if (docSnap.exists()) {
          // Changed to chapter2
          setBookmarks(docSnap.data().chapter2 || []);
        } else {
          setBookmarks([]);
        }
        setIsLoading(false);
  ...
  const toggleBookmark = async (shlokaNum) => {
    ...
    try {
      const docRef = doc(db, "userBookmarks", currentUser.uid);
      // Changed to chapter2
      await setDoc(docRef, { chapter2: newBookmarks }, { merge: true });
    } catch (error) {
  ...
  return (
    <div className="bg-gradient-to-br from-amber-50/30 via-[#F8F5F2] to-orange-50/20 dark:from-slate-950 dark:via-slate-900 dark:to-slate-950 text-slate-800 dark:text-slate-100 min-h-screen font-sans transition-colors duration-300">
      <SEO 
        title={`Bhagavad Gītā Chapter 2: सांख्ययोगः - Darshanam`} 
        description={`Explore Bhagavad Gītā Chapter 2 - Sankhya Yoga - Transcendental Knowledge. Read translations, listen to audio, and discover the meaning.`} 
      />
      <SEO 
        title={`Bhagavad Gītā Chapter 2: सांख्ययोगः - Darshanam`} 
        description={`Explore Bhagavad Gītā Chapter 2 - Sankhya Yoga - Transcendental Knowledge. Read translations, listen to audio, and discover the meaning.`} 
