...
import { chapter15Shlokas } from "../../../data/chapter15";
...
const Adhyay15 = () => {
  ...
  useEffect(() => {
    if (currentUser) {
      const fetchBookmarks = async () => {
        ...
        if (docSnap.exists()) {
          setBookmarks(docSnap.data().chapter15 || []);
        } else {
          setBookmarks([]);
        }
        setIsLoading(false);
  ...
  const toggleBookmark = async (shlokaNum) => {
    ...
    try {
      const docRef = doc(db, "userBookmarks", currentUser.uid);
      await setDoc(docRef, { chapter15: newBookmarks }, { merge: true });
    } catch (error) {
  ...
  return (
    <div className="bg-gradient-to-br from-amber-50/30 via-[#F8F5F2] to-orange-50/20 dark:from-slate-950 dark:via-slate-900 dark:to-slate-950 text-slate-800 dark:text-slate-100 min-h-screen font-sans transition-colors duration-300">
      <SEO 
        title={`Bhagavad Gītā Chapter 15: पुरुषोत्तमयोगः - Darshanam`} 
        description={`Explore Bhagavad Gītā Chapter 15 - Purushottama Yoga - The Supreme Person. Read translations, listen to audio, and discover the meaning.`} 
      />
      <SEO 
        title={`Bhagavad Gītā Chapter 15: पुरुषोत्तमयोगः - Darshanam`} 
        description={`Explore Bhagavad Gītā Chapter 15 - Purushottama Yoga - The Supreme Person. Read translations, listen to audio, and discover the meaning.`} 
