...
import { chapter18Shlokas } from "../../../data/chapter18";
...
const Adhyay18 = () => {
  ...
  useEffect(() => {
    if (currentUser) {
      const fetchBookmarks = async () => {
        ...
        if (docSnap.exists()) {
          setBookmarks(docSnap.data().chapter18 || []);
        } else {
          setBookmarks([]);
        }
        setIsLoading(false);
  ...
  const toggleBookmark = async (shlokaNum) => {
    ...
    try {
      const docRef = doc(db, "userBookmarks", currentUser.uid);
      await setDoc(docRef, { chapter18: newBookmarks }, { merge: true });
    } catch (error) {
  ...
  return (
    <div className="bg-gradient-to-br from-amber-50/30 via-[#F8F5F2] to-orange-50/20 dark:from-slate-950 dark:via-slate-900 dark:to-slate-950 text-slate-800 dark:text-slate-100 min-h-screen font-sans transition-colors duration-300">
      <SEO 
        title={`Bhagavad Gītā Chapter 18: मोक्षसंन्यासयोगः - Darshanam`} 
        description={`Explore Bhagavad Gītā Chapter 18 - Moksha Sanyasa Yoga - Liberation and Renunciation. Read translations, listen to audio, and discover the meaning.`} 
      />
      <SEO 
        title={`Bhagavad Gītā Chapter 18: मोक्षसंन्यासयोगः - Darshanam`} 
        description={`Explore Bhagavad Gītā Chapter 18 - Moksha Sanyasa Yoga - Liberation and Renunciation. Read translations, listen to audio, and discover the meaning.`} 
      ...
      >
        <div className="relative container mx-auto px-4">
          ...
          >
            Moksha Sanyasa Yoga - Liberation and Renunciation
          </motion.h2>
          <motion.div
