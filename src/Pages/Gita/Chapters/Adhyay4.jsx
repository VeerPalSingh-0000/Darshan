...
const Adhyay4 = () => {
  ...
  useEffect(() => {
    if (currentUser) {
      const fetchBookmarks = async () => {
        ...
        if (docSnap.exists()) {
          setBookmarks(docSnap.data().chapter4 || []);
        } else {
          setBookmarks([]);
        }
        setIsLoading(false);
  ...
  const toggleBookmark = async (shlokaNum) => {
    ...
    try {
      const docRef = doc(db, "userBookmarks", currentUser.uid);
      await setDoc(docRef, { chapter4: newBookmarks }, { merge: true });
    } catch (error) {
  ...
  return (
    <div className="bg-gradient-to-br from-amber-50/30 via-[#F8F5F2] to-orange-50/20 dark:from-slate-950 dark:via-slate-900 dark:to-slate-950 text-slate-800 dark:text-slate-100 min-h-screen font-sans transition-colors duration-300">
      <SEO 
        title={`Bhagavad Gītā Chapter 4: ज्ञानकर्मसंन्यासयोगः - Darshanam`} 
        description={`Explore Bhagavad Gītā Chapter 4 - Jnana Karma Sanyasa Yoga - Knowledge, Action & Renunciation. Read translations, listen to audio, and discover the meaning.`} 
      />
      <SEO 
        title={`Bhagavad Gītā Chapter 4: ज्ञानकर्मसंन्यासयोगः - Darshanam`} 
        description={`Explore Bhagavad Gītā Chapter 4 - Jnana Karma Sanyasa Yoga - Knowledge, Action & Renunciation. Read translations, listen to audio, and discover the meaning.`} 
      ...
      >
        <div className="relative container mx-auto px-4">
          ...
          <motion.p
            className="text-amber-700 dark:text-amber-600 font-bold tracking-[0.2em] text-sm uppercase mb-2"
          ...
          >
            Jnana Karma Sanyasa Yoga - Knowledge, Action & Renunciation
          </motion.h2>
          <motion.div
